...
38. **CanvasWindow** — reusable zoomable/pannable GDI canvas (`UI/CanvasWindow/CanvasWindow.h`). `create(parent, x, y, w, h)`, `redraw()`, `resetView()`, `setGridVisible()` / `setGridSpacing()` / `setGridExtent()` / `setGridColor()`, `setBackgroundColor()`, `setDefaultZoom()` / `setDefaultPan()`, `setZoomMultiplier()` / `setMinZoom()` / `setMaxZoom()` with matching getters. Subclass overrides `virtual onDraw(HDC, RECT)`. Coordinate helpers: `toScreenX(worldX)` / `toScreenY(worldY)`. World units in mm, Y-axis flipped. Mouse wheel uses multiplicative zoom per notch (defaults: 1.10, 0.02..5.0), remains centered on the cursor; left-drag = pan, double-click = reset. Child window with `GWLP_USERDATA` pattern (not a `UIComponent`).
